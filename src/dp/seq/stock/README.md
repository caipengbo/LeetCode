# 团灭 LeetCode 股票买卖问题
k代表交易次数
第一题（121题）是只进行一次交易，相当于 k = 1；
第二题（122题）是不限交易次数，相当于 k = +infinity（正无穷）；
第三题（123题）是只进行 2 次交易，相当于 k = 2；
...
### 第三题（309题），k = +infinity with cooldown
...
还记得前面总结的「穷举框架」吗？就是说我们必须穷举所有状态。其实我们之前的解法，都在穷举所有状态，只是之前的题目中 k 都被化简掉了。比如说第一题，k = 1：这道题由于没有消掉 k 的影响，所以必须要对 k 进行穷举：
